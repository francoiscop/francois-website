<template>
    <div>
        <form style="padding:20px" :id="'form-' + label" class="box elevation-8" @submit.prevent="onSubmit" enctype="multipart/form-data">
            <div v-if="step[0]">
                <slot></slot>
                <vv-radio-type
                        :label="uploadRoot.type + ':'"
                        required
                        ref="radiotype"
                        :type="uploadRoot.type"
                        @clearError="clearError($event)"
                        @change="setValueType($event)"
                ></vv-radio-type>
                <div v-for="(item, field, index) in formRoot.newData" style="margin-bottom:5px;" :class="'c-input--' + field">
...
<script>
import Thumbs from './thumbsClass'
...
export default {
    ...
    created() {
        /*
        console.log('***** FORMROOT from <form> *****');
        for (let k in this.formRoot.newData) {
            console.log(k + ':');
            console.log(this.formRoot.newData[k]);
        }
        console.log('*********************************');

        console.log('***** UPLOADROOT from <form> *****');
        for (let k in this.uploadRoot) {
            console.log(k + ':');
            console.log(this.uploadRoot[k]);
        }
        console.log(this.uploadRoot.valid_file_upload.toString());
        console.log('*********************************');
    ...
    mounted() {
        console.log('POST ACTION :', this.postAction);
    },
    ...
    methods: {
                ...
                if (newFile.success && !oldFile.success) {
                    ...
                    if (this.uploadRoot.type === 'video'){
                        ...
                        file.thumbs.start().then((response) => {
                            ...
                            this.$refs.upload.update(file, {thumbs : { process : false}, editURL : this.uploadRoot.edit + id});
                        });
                    }
                }
            }
        ...
        inputFilter: function(newFile, oldFile, prevent) {
            ...
            if (newFile && !oldFile) {
                ...
                //Filter Extension:
                var re = new RegExp('\\.(' + this.extensions.replace(/,/g,'|') + ')$', 'i');
                if (newFile.name.search(re) === -1){
        ...
        onSubmit() {
            ...
            for (let k in this.$children) {
                ...
                if (ref !== 'upload' && ref !== 'radiotype') {
                    var type = this.formRoot.newData[ref].type;
                ...
                } else if (ref === 'radiotype'){

                    
                    if (this.radioTypeData.radio === null){
                        child.setError('radio', 'REQUIRED');
                        this.errors.push('radio');
                    }

                    if (this.radioTypeData.radio === 'personal'){

                        if (!this.radioTypeData.model && !this.radioTypeData.producer){
                            child.setError('model', 'REQUIRED');
                            this.errors.push('model');
                        }
                        if (!this.radioTypeData.age){
                            child.setError('age', 'Not checked');
                            this.errors.push('age');
                        }
                        if (!this.radioTypeData.copyrights){
                            child.setError('copyrights', 'Not checked');
                            this.errors.push('copyrights');
                        }
                        if (!this.radioTypeData.terms){
                            child.setError('terms', 'Not checked');
                            this.errors.push('terms');
                        }
                        if (child.errors.price.is){
            ...
            for (let k in this.radioTypeData){
                this.data[k] = this.radioTypeData[k];
            }
