  ...
  <div id="scrollmagic">
      ...
      <div class="pinContainer" ref="pin">
          ...
          <section
              ...
              :class="[`panel-${index}`, panel.bg_class]"
              :stylexxx="{backgroundColor: panel.bg_color}"
              :style="{'z-index': index}"
              :id="`panel-${index}`"
          >
...
export default{
  ...
  components : {'v-scroll-buttons' : scrollButtonsComponent},
	...
	mounted () {
      ...
      this.debug('MOUNTED', 'this.$el.textContent', this.$el.textContent)
  ...
  computed: {
    ...
    index(){
      return store.getters['home/getIndex']
    },

    navY(){
      return store.getters['home/getNavY']
    },


    propertyComputed() {

        this.debug('COMPUTED', 'I change when this.property changes', null, 'warn', 'propertyComputed')
  ...
  beforeCreate() {
      ...
      console.error('BEFORE CREATE', 'Nothing gets called before me!')
  ...
  created() {
      ...
      this.debug('CREATED', 'propertyComputed will update, as this.property is now reactive')
  },
  ...
  methods: {
    ...
    debug(cycle, libel, debug, c = 'warn', func = null){
        ...
        console[c](`HOME.VUE ${cycle} -> func : ${func} -> ${libel} : `, debug)
    ...
    init(){
        ...
        store.dispatch('home/setPanels', this.panels)
    ...
    setScene(){
        ...
        for (var i = 0; i < Length; i++) {
          ...
          switch (i) { // Set animFrom value, depending on the index i of the item
            ...
            case 1:
              //animFrom = {y: '100%'} // Second panel comes from the left
        ...
        .on("update", (event) => {
            ...
            console.warn('scrollPos :', event.scrollPos)
            console.error('WINDOW scrollY', window.scrollY)
            //console.warn('direction :', event)
          })
        .on("enter leave", (event) => {
            //$("#state").text(e.type == "enter" ? "inside" : "outside");
            console.error('INSIDE OR OUTSIDE ?', event.type == "enter" ? "inside" : "outside")
        })
        .on("start end", (event) => {
            //$("#lastHit").text(e.type == "start" ? "top" : "bottom");
            console.error('start or end ? :', event.type == "start" ? "top" : "bottom")
        })
        .on("progress", (event) => {
            //$("#progress").text(e.progress.toFixed(3));
            console.warn(event.scrollDirection)
            //console.error(e.progress.toFixed(3))
        });
    ...
    setScrollYPositionPanel(){
        ...
        this.panels.forEach((panel, i) => {
            ...
            panel.Y = Math.round(startPosition + ((duration * i) / num))
    ...
    scroll(direction){
        ...
        store.dispatch('home/scroll', direction)

        this.debug('METHODS', 'index', this.index, 'warn', 'scroll')
    ...
    */
    ...
    listeners(){
        ...
        window.onbeforeunload = () => {
          ...
          this.debug('METHODS', 'onbeforeunload', null, 'error', 'listeners')
        ...
        window.addEventListener('scroll', debounce((e) => {
            ...
            console.warn('this.navY , window scrollY', this.navY, window.scrollY)

            if (window.scrollY === this.navY) return;

            this.debug('METHODS', 'getManualScroll', store.getters['home/getManualScroll'], 'error', 'listeners')

            if (!store.getters['home/getManualScroll']) return;


            store.commit('home/setNavY', null)
            ...
            for (let k in this.panels){
                ...
                console.warn('panel.Y ', this.panels[k].Y)

                if (window.scrollY <= this.panels[k].Y){


                    console.error('COMPONENT home.vue -> k : ', k - 1)

                    //this.index = k - 1;
                    store.commit('home/setIndex', k - 1)
    ...
    checkIndex(){
      ...
      this.debug('METHODS', 'CHECK INDEX BECAUSE MANUAL SCROLL', null, 'warn', 'checkindex')

        const actualIndex = store.getters['home/getIndex']

        let index = 0

        for (index in this.panels){

            if (index >= this.panels.length - 1) break;

            if (window.scrollY <= this.panels[+index + 1].Y) break;
        }
    ...
    setIndex(index){
        ...
        store.commit('home/setIndex', index)
    }
