<template>
  <div style="max-width: 500px;" class="center-auto">
    <div :class="aspectRatioClass">
            <div :class="{'has-background-primary' : isLoading}" style="overflow: hidden;">
    ...
    <div class="aspect__ratio--1-78 min-w-100 max-w-200 center-auto">
            <div style="background-size:cover;background-image: url('');" class="has-background-primary border-white">
              TEST
            </div>
    </div>
  ...
  <!--
  <div class="vs-player flex flex-column" :class="{'vs-player--100vh' : isCentered}">

      <div class="flex-grow flex flex-center">
          <div class="vs-player__inner has-background-black" :class="{'has-background-danger' : errorPlayer.status}" :style="'max-width:' + maxWidth + 'px'">
              <div class="vs-player__inner--16-9">
                  <div class="vs-player__padding" :style="paddingTop">

                      <div class="vs-player--error center-absolute flex flex-center" v-if="errorPlayer.status">
                          <div class="not-found--icon"></div>
                          <div class="u s-20">{{ errorPlayer.libel }}</div>
                      </div>

                      <video id="player" ref="player" v-show="!isLoading"></video>
      ...
      <div class="m-20 flex flex-center debug" v-if="editable && !errorPlayer.status &&!isLoading">
          <b-switch v-model="isEditingMode" class="s-9 tooltip is-tooltip-warning" data-tooltip="Save your own extract">
...
.vs-player__padding{
    ...
    /************************************************************************ Problem with crtain video : if the vido
    is square; */
    /*
    overflow: hidden;
}
    ...
    export default {
        ...
        created(){
            ...
            if (this.video.hasOwnProperty('aspect')){
                    ...
                    console.log('aspect__ratio--' + a[0] + '-' + a[1])
        ...
        computed : {
            ...
            paddingTop(){
                ...
                return (this.isPaddingTop()) ? 'padding-top: ' + this.video.padding + '%' : 'padding-top: 56.25%';
